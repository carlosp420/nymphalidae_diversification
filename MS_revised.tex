...
Marianne Espeland\textsuperscript{2,3,4}
\\
\bf{1} Laboratory of Genetics, Department of Biology, University of Turku, Turku, Finland
\\
\bf{2} Museum of Comparative Zoology, Harvard University, Cambridge, USA
\bf{3} Department of Organismic and Evolutionary Biology, Harvard University, Cambridge, USA
\\
...
\bf{4} Current address: McGuire Center for Lepidoptera and Biodiversity,
...
trees using Burntrees v.0.1.9 \url{http://www.abc.se/~nylander/} (S1 File)
...
phylogenies were available (S1 Dataset).
...
\cite{dyer2002, beccaloni2008, janzen2009} and others 
(S2 and S3 Dataset) for a
total of 6586
...
perform the analyses (included as S2 and S3 File). All
...
(MultiMEDUSA, S1 File). We calculated a new MCC tree from the
...
versus 176 that use more than one; based on our data in S2 Dataset),
...
1) (S4 Dataset). The family Solanaceae include the hostplants of the
...
sample of 1000 trees (S1 File) from the posterior distribution and
compared the results with a MEDUSA analysis on the MCC tree derived from
this sample (S2 Supporting Information).
...
MEDUSA on the 1000 trees are provided as an R object in S4 File.
...
(S6 Fig. and S1 Text), and found that the estimated net diversification rate
...
net diversification rate on part of Ithomiini (Oleriina + Ithomiina
...
(Table~\ref{table3};
character states available in S4 Dataset, code in S3 Supporting Information,
and mcmc run in S4 Supporting Information). We combined the post-burnin mcmc generations
...
credibility tree (combined mcmc run in S5 Supporting Information; profiles
