...
trees using Burntrees v.0.1.9 \url{http://www.abc.se/~nylander/} (S9 File)
...
phylogenies were available (S13 Dataset).
...
(MultiMEDUSA, S9 File). We calculated a new MCC tree from the
...
versus 176 that use more than one; based on our data in S14 Dataset),
...
1) (S16 Dataset). The family Solanaceae include the hostplants of the
...
sample of 1000 trees (S9 File) from the posterior distribution and
compared the results with a MEDUSA analysis on the MCC tree derived from
this sample (S19 Supporting Information).
...
MEDUSA on the 1000 trees are provided as an R object in S12 File.
...
(S6 Fig. and S23 Text), and found that the estimated net diversification rate
...
(Table~\ref{table3};
character states available in S16 Dataset, code in S20 Supporting Information,
and mcmc run in S21 Supporting Information). We combined the post-burnin mcmc generations
...
credibility tree (combined mcmc run in S22 Supporting Information; profiles
...
\subsection*{S21 Supporting Information}
#\subsection*{S19 Supporting Information}
\label{S21_Supporting_Information}
{\bf Raw results for the BiSSE analysis}.

\subsection*{S22 Supporting Information}
#\subsection*{S20 Supporting Information}
\label{S22_Supporting_Information}
