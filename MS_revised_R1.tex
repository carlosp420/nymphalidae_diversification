...
hypothesis'' {[}4,5{]} or ``diffuse cospeciation'' {[}2{]} and the
``musical chairs hypothesis'' {[}6{]}.
...
6000 species {[}7{]}, and several members are considered model organisms
in evolutionary biology {[}8--10{]}. The family most likely originated
...
{[}11{]}. Several studies have used time-calibrated phylogenies and
...
of some Nymphalidae clades {[}11--14{]}. For example, it has been
...
Satyrinae {[}15{]} due to the abundance of grasses over extensive
geographic areas (``resource abundance-dependent diversity dynamics''
hypothesis). Fordyce (2010) {[}13{]} found increased net diversification
...
hypothesis for Nymphalidae butterflies {[}14{]} to investigate patterns
of diversification. We applied the statistical method MEDUSA
{[}17,18{]}, to study the diversification pattern of Nymphalidae
...
MEDUSA method called MultiMEDUSA {[}17{]}. We also tested whether
...
et al. (2009) {[}14{]} that were generated using DNA sequence data from
...
{[}14{]}. Their original BEAST run was for 40 million generations. We
...
\url{http://tolweb.org}, Lamas (2004) {[}20{]} and curated lists of
...
{[}14{]} study according to available phylogenetic studies {[}21--32{]},
...
sources including {[}33{]}, HOSTS database (\url{http://bit.ly/YI7nwW}),
{[}34--36{]} and others (supp. mat. 15--16) for a total of 6586
...
We used the statistical software R version 3.0.1 {[}37{]} in combination
with the APE {[}38{]}, GEIGER {[}39{]}, MEDUSA {[}17{]} and
\texttt{diversitree} {[}19{]} packages along with our own scripts to
...
{[}14{]}, on the maximum clade credibility tree derived from these and
the MCC tree {[}14{]}.
...
analysis of macroevolutionary mixtures) {[}40{]} is a software that fits
...
version 093.4.33 {[}17{]} on the maximum clade credibility tree from
Wahlberg et al. (2009) {[}14{]}. MEDUSA fits alternative birth-death
...
account branch lengths and number of species per lineage {[}17{]}. Most
...
distribution of the Bayesian run from Wahlberg et al., 2009 {[}14{]}
...
extinction'' {[}41{]} Bayesian approach as implemented in the R package
\texttt{diversitree} {[}19{]}. The Markov Chain Monte Carlo algorithm
was run for 10000 generations discarding the first 7500 as burnin. The
Multiple State Speciation and Extinction approach (MuSSE) {[}19{]} is
...
diversification rate for a clade {[}41{]}. We used our compiled data of
...
diverse Ithomiini butterflies and closest relatives {[}9{]}. Other
...
feeding on Solanaceae {[}35,42{]} might be incorrect as it is unlikely
...
been shown that BiSSE performs poorly under certain conditions {[}43{]}.
...
in the software BAMM {[}40{]}. We found that the best model (best
...
rates in a phylogenetic tree. Since its publication {[}17{]} the results
...
{[}11,44,45{]}. However, different diversification shifts and different
...
{[}31{]}. Thus, the Bayesian runs are expected to recover alternative
...
{[}14{]}. As a result, MEDUSA inferred a net diversification rate
...
posterior probability values in Wahlberg et al. (2009) {[}14{]}.
...
in the diversification of Ithomiini butterflies {[}46{]}. Ithomiini
butterflies are exclusively Neotropical and most species feed on
Solanaceae hostplants during larval stage {[}9{]}. Optimizations of the
evolution of hostplant use on phylogenies evidence a probable shift from
Apocynaceae to Solanaceae in the ancestor of the tribe {[}9,47{]}.
Fordyce (2010) {[}13{]} found that the Gamma statistics, a LTT plot of
...
this, we used a Bayesian approach {[}48{]} to test whether the trait
...
{[}13{]}. Due to the fact that Ithomiini are virtually the only
...
hostplant character. As noted by Maddison et al. (2007) {[}41{]}, the
...
``escape-and-radiate scenario'' due to a shift onto Solanaceae {[}13{]}
and radiation scenarios among chemically different lineages of
Solanaceae plants {[}9,46{]}. According to this theory, the shift from
...
known {[}9{]}, we do not have any understanding on the physiological
...
exploiting a novel toxic resource {[}9,14{]} might not be too efficient
...
hostplant switch {[}50{]}. This might be the reason why the basal
Ithomiini subtribes Melinaeina and Mechanitina are so species-poor and
restricted to few Solanaceae hosts {[}9{]}, while recent subtribes are
species-rich and have expanded their host range into several Solanaceae
lineages {[}9{]}. It might be that the switch to feeding in Solanaceae
...
{[}49{]} found strong evidence for a model of speciation congruent with
...
Angiosperms {[}51{]}, the family Solanaceae split from its sister group
about 59 (49--68) MYA and diversification started (crown group age)
around 37 (29--47) MYA. Wahlberg et al. (2009) {[}14{]} give the ages
...
cooling in the Oligocene {[}15{]}. Thus, it is somewhat surprising that
...
attributed to low phylogenetic signal {[}14{]}, the clade Satyrini is
very robust {[}14{]} and MEDUSA failed to identify any significant
...
MCC Nymphalidae tree from {[}14{]}. This script removes the outgroup
...
6. Hardy NB, Otto SP (2014) Specialization and generalization in the
diversification of phytophagous insects: Tests of the musical chairs and
oscillation hypotheses. P Roy Soc B 281: 20132960.

7. Nieukerken EJ van, Kaila L, Kitching IJ, Kristensen NP, Lees DC, et
...
8. Joron M, Jiggins C, Papanicolaou A, McMillan W (2006) Heliconius wing
patterns: An evo-devo model for understanding phenotypic diversity.
Heredity 97: 157--167.

9. Willmott KR, Freitas AVL (2006) Higher-level phylogeny of the
...
10. Brakefield PM, Beldade P, Zwaan BJ (2009) The african butterfly
Bicyclus anynana: A model for evolutionary genetics and evolutionary
developmental biology. Cold Spring Harb Prot 2009: pdb--emo122.

11. Heikkil\"a M, Kaila L, Mutanen M, Pe\~na C, Wahlberg N (2012) Cretaceous
...
12. Elias M, Joron M, Willmott KR, Silva-Brand\~ao KL, Kaiser V, et al.
...
13. Fordyce JA (2010) Host shifts and evolutionary radiations of
butterflies. P Roy Soc B 277: 3735--3743.
doi:\href{http://dx.doi.org/10.1098/rspb.2010.0211}{10.1098/rspb.2010.0211}.

14. Wahlberg N, Leneveu J, Kodandaramaiah U, Pe\~na C, Nylin S, et al.
...
15. Pe\~na C, Wahlberg N (2008) Prehistorical climate change increased
...
16. Ferrer-Paris JR, S{\'a}nchez-Mercado A, Viloria \'AL, Donaldson J (2013)
...
17. Alfaro ME, Santini F, Brock C, Alamillo H, Dornburg A, et al. (2009)
...
18. Harmon LJ, Rabosky DL, {FitzJohn} RG, Brown JW (2011) turboMEDUSA:
MEDUSA: Modeling Evolutionary Diversification Using Stepwise AIC.
Available: \url{https://github.com/josephwb/turboMEDUSA}.

19. {FitzJohn} RG (2012) Diversitree: comparative phylogenetic analyses
...
20. Lamas G (2004) Checklist: Part 4A. Hesperioidea-Papilionoidea.
Gainesville: Association for Tropical Lepidoptera; Scientific
Publishers.

21. Matos-Marav\'i PF, Pe\~na C, Willmott KR, Freitas AV, Wahlberg N (2013)
...
22. Brower AVZ, Wahlberg N, Ogawa JR, Boppr\'e M, Vane-Wright RI (2010)
...
23. Kodandaramaiah U, Lees DC, M\"uller CJ, Torres E, Karanth KP, et al.
...
24. Kodandaramaiah U, Pe\~na C, Braby MF, Grund R, M\"uller CJ, et al.
...
25. Ortiz-Acevedo E, Willmott KR (2013) Molecular systematics of the
...
26. De-Silva DL, Day JJ, Elias M, Willmott KR, Whinnett A, et al. (2010)
...
27. Freitas AVL, Brown Jr. KS (2004) Phylogeny of the Nymphalidae
(Lepidoptera). Syst Biol 53: 363--383.
doi:\href{http://dx.doi.org/10.1080/10635150490445670}{10.1080/10635150490445670}.

28. Pe\~na C, Wahlberg N, Weingartner E, Kodandaramaiah U, Nylin S, et al.
...
29. Penz CM (1999) Higher level phylogeny for the passion-vine
...
30. Silva-Brand\~ao KL, Wahlberg N, Francini RB, Azeredo-Espin AML, Brown
...
31. Pe\~na C, Nylin S, Wahlberg N (2011) The radiation of Satyrini
...
32. Pe\~na C, Nylin S, Freitas AVL, Wahlberg N (2010) Biogeographic
...
33. Ackery PR (1988) Hostplants and classification: A review of
...
34. Dyer LA, Gentry GL (2002) Caterpillars and parasitoids of a tropical
lowland wet forest. Available: \url{http://www.caterpillars.org}.
Accessed 17 April 2013.

35. Beccaloni G, Viloria \'A, Hall S, Robinson G (2008) Catalogue of the
hostplants of the Neotropical butterflies. London: The Natural History
Museum.

36. Janzen DH, Hallwachs W (2009) Dynamic database for an inventory of
...
37. Core Team R (2013) R: A Language and Environment for Statistical
Computing. Vienna, Austria: R Foundation for Statistical Computing.
Available: \url{http://www.r-project.org}.

38. Popescu A-A, Huber KT, Paradis E (2012) ape 3.0: New tools for
...
39. Harmon LJ, Weir JT, Brock CD, Glor RE, Challenger W (2008) GEIGER:
...
40. Rabosky DL (2014) Automatic detection of key innovations, rate
shifts, and diversity-dependence on phylogenetic trees. PLoS ONE 9:
e89543. Available:
\url{http://dx.doi.org/10.1371\%2Fjournal.pone.0089543}.

41. Maddison WP, Midford PE, Otto SP (2007) Estimating a binary
...
42. Scott JA (1986) The Butterflies of North America. Stanford
University Press.

43. Davis MP, Midford PE, Maddison W (2013) Exploring power and
...
44. Litman JR, Danforth BN, Eardley CD, Praz CJ (2011) Why do leafcutter
...
45. Ryberg M, Matheny PB (2012) Asynchronous origins of ectomycorrhizal
clades of Agaricales. P Roy Soc B 279: 2003--2011.
doi:\href{http://dx.doi.org/10.1098/rspb.2011.2428}{10.1098/rspb.2011.2428}.

46. Brown Jr. KS (1987) Chemistry at the Solanaceae/Ithomiinae
Interface. Ann Mo Bot Gard 74: 359--397.

47. Brower AVZ, Freitas AVL, Lee M-M, Silva-Brand\~ao KL, Whinnett A, et
...
48. {FitzJohn} RG, Maddison WP, Otto SP (2009) Estimating
...
49. Wheat CW, Vogel H, Wittstock U, Braby MB, Underwood D, et al. (2007)
...
50. Li W, Schuler MA, Berenbaum MR (2003) Diversification of
...
51. Bell CD, Soltis DE, Soltis PS (2010) The age and diversification of
the angiosperms re-revisited. Am J Bot 97: 1296--1303.
doi:\href{http://dx.doi.org/10.3732/ajb.0900346}{10.3732/ajb.0900346}.

52. Wahlberg N, Rubinoff D (2011) Vagility across Vanessa (Lepidoptera:
