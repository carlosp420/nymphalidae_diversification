install.packages("code/auteur_0.12.1010.tar.gz", repos=NULL, type="source")
library(turboMEDUSA)
library(ape)
library(auteur)
library(phytools)
...
# read in trees
for( i in 1:10 ) {
    this_file <- paste("output/set_", i, ".nex", sep="")
    mct_file  <- paste(this_file, "_mct.nex", sep="")
    print(this_file)
    print(mct_file)



dfg <- read.nexus(this_file); # trees from MrBayes run
tax <- read.csv(file="data/supp_mat_03_richness.csv"); # two column dataframe of genera (match tip.lables) and species richness
mct <- read.nexus(mct_file); # mct is the maximum credibility tree
...
output
write.table(output, file=paste(this_file, "raw_data_summary.csv", sep="_"), row.names=FALSE)
...
write.table(d, file=paste(mct_file, "raw_data.csv", sep="_"), row.names=FALSE)
